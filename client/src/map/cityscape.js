...
// Builds ONE BUILDING'S LOT, seen from above — the citizens view's counterpart to
// buildRotaryTown. Same flat top-down space (plain scene pixels, no tile grid, no
// iso projection): the building sits in the middle of a grassy lot with a driveway
// out to the road, a fence, trees and lamps, and a closed footpath the citizens
// stroll. The building is drawn roofless so the desks inside are visible — the
// roof-off reveal a top-down game does when you step indoors.
// Deterministic from the roster count so it is stable across renders.
export function buildLot(count, { perRow = 4 } = {}) {
  const items = Math.max(count, 0);
  const total = items + 1;                       // + the "add citizen" desk
  const cols = Math.min(perRow, Math.max(2, total));
  const rows = Math.ceil(total / cols);

  const DESK_W = 76, DESK_H = 44;                // one desk's footprint
  const AISLE_X = 52, AISLE_Y = 62;              // gaps between desks
  const PADIN = 52;                              // clearance inside the walls
  const WALL = 13;                               // wall thickness
  const YARD = 168;                              // grass around the building
  const DRIVE = 104;                             // driveway length below the lot

  const innerW = cols * DESK_W + (cols - 1) * AISLE_X + PADIN * 2;
  const innerH = rows * DESK_H + (rows - 1) * AISLE_Y + PADIN * 2;
  const bW = innerW + WALL * 2;
  const bH = innerH + WALL * 2;

  const width = Math.round(bW + YARD * 2);
  const height = Math.round(bH + YARD * 2 + DRIVE);
  const bx = YARD, by = YARD;                    // building's top-left corner
  const building = { left: bx, top: by, w: bW, h: bH, wall: WALL };

  // Desks in tidy rows; the LAST one is left empty as the "add citizen" lot.
  const cells = [];
  for (let k = 0; k < total; k += 1) {
    const c = k % cols, r = Math.floor(k / cols);
    cells.push({
      left: Math.round(bx + WALL + PADIN + c * (DESK_W + AISLE_X) + DESK_W / 2),
      top: Math.round(by + WALL + PADIN + r * (DESK_H + AISLE_Y) + DESK_H / 2),
    });
  }
  const withZ = (p) => ({ ...p, z: Math.round(200 + p.top) });
  const slots = cells.slice(0, items).map(withZ);
  const addSlot = withZ(cells[items]);

  // Indoor furnishing, so the revealed floor reads as a room. The bands just
  // inside the top and bottom walls are always free — desks start PADIN in — so
  // cabinets line the back and a rug sits by the door.
  const cabinets = [];
  for (let c = 0; c < cols; c += 1) {
    cabinets.push(withZ({
      left: Math.round(bx + WALL + PADIN + c * (DESK_W + AISLE_X) + DESK_W / 2),
      top: Math.round(by + WALL + 22),
    }));
  }
  const rug = withZ({ left: Math.round(bx + bW / 2), top: Math.round(by + bH - WALL - 30) });

  // Front door, centred on the bottom wall, with a driveway running to the road.
  const door = { left: Math.round(bx + bW / 2), top: by + bH, w: 62 };
  const road = { y: height - 34 };
  const drive = { x1: door.left, y1: door.top, x2: door.left, y2: road.y };

  // The footpath the citizens stroll: a closed ring around the building, out in
  // the grass, clear of both the walls and the fence.
  const M = 74;
  const ring = [
    [bx - M, by - M], [bx + bW + M, by - M],
    [bx + bW + M, by + bH + M], [bx - M, by + bH + M],
  ].map(([x, y]) => `${Math.round(x)} ${Math.round(y)}`);
  const walkLoop = `M ${ring.join(' L ')} Z`;

  // Fence along the lot boundary, with a gap where the driveway crosses it.
  const FI = 30;                                  // inset from the scene edge
  const fx0 = FI, fy0 = FI, fx1 = width - FI, fy1 = height - DRIVE + 22;
  const fence = [];
  for (let x = fx0; x <= fx1 - 34; x += 34) {
    fence.push({ left: x, top: fy0, w: 34, dir: 'h' });
    if (Math.abs(x + 17 - door.left) > door.w) fence.push({ left: x, top: fy1, w: 34, dir: 'h' });
  }
  for (let y = fy0; y <= fy1 - 34; y += 34) {
    fence.push({ left: fx0, top: y, w: 34, dir: 'v' });
    fence.push({ left: fx1, top: y, w: 34, dir: 'v' });
  }

  // Yard scenery, kept out of the stroll ring and off the driveway.
  const trees = [
    { left: fx0 + 54, top: fy0 + 58 }, { left: fx1 - 54, top: fy0 + 58 },
    { left: fx0 + 54, top: fy1 - 66 }, { left: fx1 - 54, top: fy1 - 66 },
    { left: fx0 + 52, top: Math.round(by + bH / 2) }, { left: fx1 - 52, top: Math.round(by + bH / 2) },
  ].map(withZ);
  const bushes = [
    { left: bx - 40, top: by - 30 }, { left: bx + bW + 40, top: by - 30 },
    { left: bx + bW + 40, top: by + bH + 34 }, { left: bx - 40, top: by + bH + 34 },
  ].map(withZ);
  const lamps = [
    { left: door.left - 74, top: door.top + 44 }, { left: door.left + 74, top: door.top + 44 },
    { left: bx - 52, top: by - 52 }, { left: bx + bW + 52, top: by - 52 },
  ].map(withZ);

  return { width, height, building, slots, addSlot, cabinets, rug, door, drive, road, walkLoop, fence, trees, bushes, lamps };
}
